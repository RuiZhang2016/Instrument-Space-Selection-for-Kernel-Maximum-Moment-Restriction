...
def experiment(sname, seed, datasize, nystr=False):
    ...
    def normalized(params, W):
        al, bl = np.exp(params)
        L = bl * bl * np.exp(-L0 / al / al / 2) + 1e-6 * EYEN

        # H = EYEN - 1/X.shape[0]*np.ones(EYEN.shape)
        # W = H @ W @ H
        ...
        # print(np.mean((c_y@c_y.T).flatten(),axis=0))
        # print((c_y).T@W@(c_y)/np.sum(W.flatten())/np.mean((c_y@c_y.T).flatten(),axis=0))
        return (c_y).T@W@(c_y)
    ...
    def callback0(params, timer=None):
        ...
        _,val = np.linalg.slogdet(W)
        print('log margin',-(L@alpha-Y).T@W_inv@(L@alpha-Y)/2-1/2*val-L.shape[0]/2*np.log(2*np.pi), 'test error', test_err)
